...
- **배경**: 바로 앞 항목에서 조준선을 안 가리려고 안내 패널을
  `GuideUIFollow.localOffset.y`를 `-0.1`(눈높이 약간 아래)에서
  `0.35`(눈높이 꽤 위)로 올렸는데, 사용자가 "고개를 움직였을 때 UI를
  확인할 수가 없어"라고 보고함.
- **처음 잘못 짚은 진단과 실패한 "수정"**: 처음엔 "고개를 숙이면(피치)
  패널이 같이 회전해서 화면 밖으로 나가는 것"이라고 추측하고,
  `GuideUIFollow`를 "좌우 회전(Yaw)만 따라가고 위/아래는 고정" 방식으로
  바꿔봤다. Play Mode에서 카메라를 40도 아래로 기울여 스크린샷을 찍어보니
  **패널이 완전히 안 보이는 걸 확인** — 이 "수정"이 오히려 새로운 버그를
  만든 것. 바로 원래(완전 회전 추종) 코드로 되돌리고, 같은 40도 테스트를
  다시 해보니 **원래 코드는 이미 정상적으로 화면에 유지되고 있었다**
  (수학적으로도 당연함 — 카메라의 회전을 그대로 복사하면 오프셋 지점은
  항상 카메라 시야의 같은 상대 위치에 있다).
- **진짜 원인 (사용자가 구체적으로 설명해줌)**: "UI를 확인하려고 위를
  보면 UI도 같이 위로 올라가 UI를 확인할 수 없다." — 이게 핵심이었다.
  **완전히 헤드락(머리 회전을 그대로 복사)된 UI는 카메라 시야 기준으로
  항상 정확히 같은 상대 각도(오프셋)에 고정되어 있어서, 그 UI를 "보려고"
  고개를 그 방향으로 돌리는 행위 자체가 UI를 똑같이 그만큼 더 회전시켜
  버린다 — 신기루처럼 아무리 쫓아가도 항상 같은 거리만큼 벗어나 있다.**
  오프셋이 0(정면 중앙)이면 이런 문제가 없지만, `0.35`처럼 큰 오프셋을
  주면 "위쪽에 있다는 느낌"은 나는데 정작 눈으로 좁혀서 볼 수는 없는
  상태가 됨. 즉 "화면 밖으로 나가서 안 보이는" 게 아니라 "잡을 수 없는
  위치에 계속 떠 있어서 안 보이는" 것이었다.
- **결정**: `localOffset`을 `(0, 0.02, 1.3)`으로 되돌림(거의 시야 정중앙,
  고개를 어느 방향으로도 움직일 필요 없이 항상 즉시 보임). 조준선을 안
  가리는 목적은 오프셋을 크게 두는 대신 **패널을 화면 정중앙 가까이 두고,
  대신 가리는 문제는 필요하면 나중에 다른 방법(예: 패널을 약간 더 멀리
  두거나, 조준 시에는 잠깐 투명해지게 하는 등)으로 접근**해야 한다는 걸
  이번에 배움. 헤드락 UI에서는 "위/아래로 옮기기"가 절대 답이 될 수 없다.
- **검증**: Play Mode 정면 스크린샷에서 안내 패널과 화살표가 정중앙에
  바로 보임(고개를 움직일 필요 없음). EditMode 테스트 11개 통과, 콘솔
  에러 0건.
- **다음에 헤드락 UI를 다시 만질 때 기억할 것**: 오프셋을 조정해서
  "어딘가를 가리키게" 하려는 시도 자체가 잘못된 접근이다. 헤드락 UI가
  항상 눈에 보이게 하려면 오프셋을 0에 가깝게 유지해야 하고, 다른 목적
  (조준선 안 가리기 등)이 있다면 오프셋이 아닌 다른 메커니즘(투명도,
  거리, 일시적 숨김 등)으로 풀어야 한다.

## 2026-09-11 — 화살표 모양 개선, 가이드 패널을 조준선 위로 이동, 컨트롤러 버튼 안내 툴팁 비활성화

- **배경**: 사용자 피드백 4가지 — (1) 화살표가 삼각형처럼 보여서 실제
  화살표 모양으로, (2) 화살표를 안내 패널 위쪽으로, (3) 안내 패널이
  컨트롤러가 가리키는 걸 가리지 않게, (4) "컨트롤러 안내 UI" 제거.
  (4)가 뭘 가리키는지 처음엔 불명확했는데, 지금까지 모든 스크린샷에
  손 근처로 떠 있던 "Grab"/"Turn" 같은 작은 라벨 — XR Template 기본
  제공 `Affordance Callouts Left/Right`(버튼 조작법 툴팁)였음을
  `Canvas` 컴포넌트로 씬을 훑어서 특정함.
- **결정 1 (화살표 모양)**: `GoalIndicatorArrow`의 텍스처 생성 로직을
  단순 삼각형에서 "몸통(사각형) + 화살촉(삼각형)" 형태로 변경. 높이의
  아래 50%는 고정 폭 몸통, 위 50%는 폭이 0으로 좁아지는 화살촉.
- **결정 2 (배치)**: `GoalArrow`의 anchor/pivot을 우상단 구석(패널 안,
  텍스트와 겹침)에서 **패널 상단 중앙 바로 위(패널 바깥, anchor
  top-center, pivot bottom-center, `anchoredPosition=(0,20)`)**로 이동.
- **결정 3 (조준선 안 가리기)**: `GuideUIFollow.localOffset`을
  `(0,-0.1,1.3)`(눈높이보다 약간 아래, 정면)에서 `(0,0.35,1.3)`(눈높이보다
  꽤 위)로 올림. 안내 패널 전체가 시야 위쪽으로 이동해서, 사람이 보통
  손을 뻗어 조준하는 정면~아래 영역을 가리지 않게 됨. 화살표는 그 위에
  더 얹혀서 패널 맨 위쪽에 뜬다.
- **결정 4 (컨트롤러 안내 툴팁 제거)**: 공용 프리팹에서 `Affordance
  Callouts Left`/`Affordance Callouts Right`(각각 Grab/Click/Turn/Blink/
  Smooth 등 여러 개의 `Lazy Tooltip` 자식을 가진 컨테이너)를 완전히
  **비활성화**(`SetActive(false)`, 삭제는 안 함 — 나중에 되돌리기 쉽도록,
  `UniversalController` 때와 동일한 패턴). `SampleScene`/`TestMap_Quest`
  양쪽에 자동 반영됨.
  - 이 비활성화의 부작용으로 씬 파일에 `Tooltip Canvas`의 `m_SizeDelta`가
    `(0,0)→(80,46)`으로 바뀌는 아주 작은 diff가 생겼는데, 이건 지난
    세션(`2026-09-04`)에 이미 기록해 둔 "Tooltip Canvas의 Content Size
    Fitter가 저장 시점마다 값이 불안정하게 재계산되는" 알려진 현상이고,
    지금은 그 툴팁 자체가 비활성화돼 있어 화면에 보이지도 않으므로 기능적
    영향 없음.
- **검증 결과 (Play Mode 스크린샷)**: 화살표가 몸통+화살촉이 있는 진짜
  화살표 모양으로 보이고, 안내 패널 위쪽 바깥에 떠 있음. 패널 전체가
  눈높이보다 위로 이동해서 정면 하단부가 트여있음. 손 근처의 Grab/Turn
  툴팁 라벨이 더 이상 안 보임(이전 스크린샷들과 비교해서 확인). 콘솔 에러
  0건, EditMode 테스트 11개 통과.
  - **미검증**: 실제로 컨트롤러 레이가 안내 패널에 가려지지 않는지는
    실기기에서 직접 조준해봐야 확실함(Editor에서는 손/컨트롤러가 카메라와
    겹쳐 있어 조준선 자체를 시각적으로 확인하기 어려움).
...
- **배경**: 작은 구(球) 캡을 붙였는데도 사용자가 "회전하면 손목이 늘어나고,
  손목을 아예 자연스럽게 없앨 수는 없을까"라고 재보고 — 구가 너무 작아서
  늘어나는 부분을 충분히 못 가렸을 가능성이 큼.
- **결정**: 구 대신 **캡슐(Capsule) Primitive**로 교체. 캡슐의 긴 축(로컬
  Y)이 기본이라, `rotation=(90,0,0)`으로 돌려서 긴 축이 손가락 반대
  방향(뒤쪽, 팔이 이어질 방향)을 향하게 함. 처음엔 반지름/길이를
  과대추정해서(스케일 0.09 균등) 손보다 훨씬 큰 풍선처럼 나와서, 스크린샷
  확인 후 반지름을 줄이고(`localScale=(0.06, 0.09, 0.06)`, 반지름
  ≈0.03) 위치도 손 쪽으로 더 겹치게(`localPosition.z=-0.02`, 캡슐 앞쪽
  끝이 손목 안쪽까지 파고들도록) 조정. 이음새가 훨씬 자연스러워짐.
  - **작업 중 발견한 사고**: 캡 오브젝트를 프리팹 Stage에서 만들고 재질을
    `Free VR Hands_M`으로 지정했는데, 이후 위치/스케일만 다시 조정하려고
    Stage를 열었다가 저장했을 때 **재질이 조용히 기본 `Lit`(URP 기본
    머티리얼)로 되돌아가 있는 걸 발견**함(원인 불명 — Prefab Stage를
    다시 열고 닫는 과정에서 MeshRenderer의 재질 참조가 유실된 것으로
    추정). `leftMr.sharedMaterial.name`으로 직접 확인해서 잡아냈고, 다시
    할당 후 저장 즉시 재확인하는 습관으로 재발 방지. **다음에 Prefab
    Stage를 여러 번 열고 닫으며 작업할 때는 매번 재질/주요 프로퍼티를
    재확인할 것.**
  - **씬 파일에 남은 무해한 잔재**: `TestMap_Quest.unity`의 `git diff`에
    이 세션 중 생성했다가 지운 이전 버전(구 캡)의 흔적으로 보이는, 존재하지
    않는 대상을 가리키는 낡은 머티리얼 오버라이드 4개(`Default-Material`
    참조)가 계속 남아있음. `PrefabUtility.GetPropertyModifications`로
    조회하면 이 4개는 안 보이고(즉 API 레벨에서는 이미 무해/비활성) 실제
    렌더링도 정상(`Free VR Hands_M`)이라 **기능적으로는 전혀 영향 없음**.
    `SetPropertyModifications`로 되돌려써도 파일에서 안 지워짐 — Unity
    내부적으로 별도 경로(아마 `m_RemovedComponents`나 이전 인스턴스 ID
    잔재)로 저장되는 것으로 추정. YAML을 직접 손으로 고치는 위험
    (CLAUDE.md §19.2 "Unity YAML 구조를 이해하지 못한 상태에서 수동
    편집 금지")을 감수할 만큼 중요하지 않다고 판단해 **그대로 남겨둠**.
    다음에 이 씬 파일을 열 때 `git diff`에서 이 4줄짜리 블록이 보여도
    당황하지 말 것 — 알고 있는, 무해한 잔재임.
  - 공용 프리팹에 반영, EditMode 테스트 11개 통과, 콘솔 에러 0건.
- **미검증**: 이번 확장된 캡이 실제 고무줄 현상을 얼마나 가려주는지는
  실기기에서 확인 필요.

## 2026-09-11 — 손목 "고무줄" 현상: 회전 문제가 아니라 스키닝 한계 — 손목 캡으로 가림

- **배경**: 이중 미러를 고쳐서 방향/모양이 맞았는데도 사용자가 "이제
  방향이랑 손은 괜찮은데 손목 부분의 고무줄 현상은 그대로"라고 보고 —
  즉 고무줄 현상은 회전값이나 미러 문제와 **무관한, 별개의 원인**이었음.
  사용자가 "손목을 아예 없앨 수는 없을까"라고 직접 제안함.
- **원인 재확인**: `SM_HandsVR_Male` 메쉬의 본 웨이트를 직접 확인 — 정점
  1942개 중 1046개(약 54%)가 2개 이상의 본에 블렌딩되어 있음. 본은 전부
  `J_Left`/`J_Right`와 그 손가락 자식들뿐이라(별도의 팔뚝/고정 본 없음),
  이론적으로는 전체가 하나의 리지드 그룹으로 같이 움직여야 하는데, 실기기
  에서 실제 컨트롤러 회전 속도/트래킹 갱신 방식과 맞물려 손목 경계
  근처에서 시각적으로 늘어나 보이는 현상이 생기는 것으로 추정(정확한
  프레임 단위 원인은 Editor에서 재현이 안 돼서 확정하지 못함 — 저가형
  무료 에셋의 리깅 품질 한계일 가능성이 높음).
- **결정**: 원인을 완전히 고치는 대신, 사용자 제안대로 **손목 경계 자체를
  안 보이게 가리는** 방향으로 해결. `Left Controller`/`Right Controller`
  (리지드, 스키닝 전혀 안 됨 — 컨트롤러 트래킹을 그대로 따라가므로 절대
  늘어날 수 없음) 밑에 `LeftWristCap`/`RightWristCap`(단순 Sphere
  Primitive, scale `(0.05,0.05,0.06)`, 손 메쉬와 같은 머티리얼
  `Free VR Hands_M` 적용, Collider는 시각 전용이라 제거)을 자식으로 추가.
  본체 손 메쉬가 늘어나도 그 이음새 부분을 리지드 구(球)로 덮어서 눈에
  안 보이게 함.
  - 공용 프리팹(`Complete XR Origin Set Up Hands Variant`)에 직접 추가해서
    `SampleScene`/`TestMap_Quest` 양쪽에 자동 반영됨.
  - Editor 스크린샷(여러 각도)으로 이음새 없이 자연스럽게 덮이는 것 확인.
    색상도 동일 머티리얼이라 잘 어울림.
  - EditMode 테스트 11개 통과, 콘솔 에러 0건.
- **미검증**: 이 캡이 실제 고무줄 현상을 얼마나 가려주는지는 실기기에서
  확인 필요. 캡 자체는 리지드라 늘어날 수 없지만, 그 안쪽에서 실제 메쉬가
  여전히 늘어나고 있다면 캡 경계 밖으로 살짝 삐져나와 보일 가능성도
  있음 — 캡 크기(현재 반지름 약 0.05~0.06)를 실기기 확인 후 키워야 할
  수도 있다.

## 2026-09-11 — 근본 원인 발견: 오른손에 걸린 "이중 미러"가 왼손처럼 보이게 만들고 있었다

- **배경**: 최소 변경(Z축 180도 추가)도 실패 — 사용자가 "손가락은 UI창을
  바라봐야 하고 손등은 하늘을 봐야 해, 여전히 두손 다 왼손이야"라고 재보고.
  회전값을 이리저리 바꿔도 "왼손처럼 보인다"는 근본 문제가 안 풀리는 걸
  보고, 이건 회전 문제가 아니라 **모양(chirality) 자체의 문제**라고
  판단해서 처음부터 완전히 다시 조사함.
- **결정적 발견**: `J_Left`/`J_Right`를 각각 로컬 회전 identity로 놓고
  손끝/엄지/새끼 본의 실제 월드 포지션을 측정해서 비교한 결과 —
  **`J_Right`(미러 스케일 `-1,1,1` 활성 상태)의 identity 포즈 데이터가
  `J_Left`의 identity 포즈 데이터와 정확히 "180도 회전" 관계였다(단순
  반사가 아니라 순수 회전 관계).** 즉 원본 메쉬의 오른손 부위 데이터는
  이미 스스로 올바르게 미러링되어 있는데, `Right Controller Visual`의
  `-1` X 스케일이 **그걸 또 한 번 미러링해서 원래대로 되돌려버리고
  있었다** — 정확히 "이중 미러 = 미러 없음"이 된 것. 이게 회전값을 아무리
  바꿔도 "왼손처럼 보이는" 게 안 없어졌던 근본 원인이었다.
  - 수학적으로: 미러가 걸린 상태에서 L/R identity 데이터 관계 = (순수
    회전, 반사 없음, determinant +1) → 오른손이 왼손의 "회전된 복사판"이
    될 뿐 진짜 미러가 아님. 미러를 제거하면 관계 = (반사 1개 포함, 순수
    미러, determinant -1) → 이게 진짜 해부학적 미러 관계.
- **검증 방법 (스크린샷 대신 손가락 구부림 방향 실측 사용)**: 손등/손바닥을
  스크린샷으로 구분하는 방식이 이미 두 번 틀렸기 때문에, 이번엔 **손가락을
  실제로 구부려서(로컬 X축 +60도, `FingerCurlAnimator`가 쓰는 것과 동일한
  축) 손끝이 어느 방향으로 움직이는지 측정**하는 방식으로 손등/손바닥을
  판별함(손가락은 항상 손바닥 쪽으로 구부러진다는 확실한 해부학적 사실
  이용). 왼손은 +Z 방향으로 구부러짐 확인(=+Z가 손바닥) → 오른손(미러
  제거 후)은 -Z 방향으로 구부러짐 확인(=+Z가 손등, 왼손과 반대) — 이
  실측으로 회전 목표를 다시 계산함.
  - **첫 번째 재계산 시도는 또 틀림**: 처음엔 이전과 같은 `R = Pᵗ·T`
    공식을 그대로 썼다가 실측(손가락 방향)이 여전히 틀리게 나옴. 원인은
    `J_Right` 자체에 스케일 `(-1,1,1)`을 준 상태이므로 `로컬 회전 → 월드
    방향` 변환 공식에 스케일이 개입해서 단순 `Pᵗ·T`로는 안 풀림(이 스케일이
    회전 앞뒤로 끼어들어서 수식이 더 복잡해짐). `S·(Pᵗ·T)·S` 형태로
    보정해도 여전히 틀림 — 결국 **분석적으로 깔끔하게 못 풀어서, 후보
    회전에 로컬 X/Y/Z 180도 회전을 하나씩 곱해보며 실측(손가락 방향)이
    맞는 걸 찾는 방식(무차별 대입)으로 정답을 찾음.**
- **최종 값 (실측으로 검증, 이번엔 스크린샷도 명확하게 일치)**:
  - `J_Left.localRotation = (90, 180, 0)` (변경 없음, 기존 확인된 값).
  - `J_Right.localScale = (-1, 1, 1)` (부모의 미러를 상쇄 → `lossyScale
    = (1,1,1)`) **— 이번에 새로 추가된 핵심 수정.**
  - `J_Right.localRotation = (275.44, 71.50, 101.67)`.
  - 실측 결과: `fingerL`/`fingerR` 둘 다 월드 `(≈0,0,1)`로 정확히 일치
    (둘 다 UI 방향을 향함). `thumbPinkyL≈(0.13,0.02,-0.06)` vs
    `thumbPinkyR≈(-0.14,0.03,-0.04)` — X 성분이 정확히 반대 부호 →
    엄지가 서로 마주보는 진짜 미러 쌍임을 확인.
  - 스크린샷(정확히 위에서 내려다본 두 손 나란히 배치)으로도 양손 다
    손등이 위로 보이고, 엄지가 서로 안쪽을 향하는 자연스러운 대칭 모양으로
    확인됨 — 이번엔 수치와 시각 확인이 둘 다 일치함.
  - EditMode 테스트 11개 통과, 콘솔 에러 0건.
- **고무줄(안대지) 현상 관련 기대**: 이전 두 번의 실패는 회전값이 커서
  생긴 게 아니라, **미러가 걸린 채로 회전만 바꾸다 보니 반사+회전이
  뒤섞여 비정상적인 조합이 됐던 것**일 가능성이 높다고 재해석함. 순수
  회전(반사 없음)은 항상 형태를 보존하므로, 이번처럼 미러를 먼저 제대로
  없애고 나면 회전값이 커도(이번 값도 원래 값과는 많이 다름) 안대지
  현상이 없어야 한다고 예상함. **다만 이건 추론이고 확정은 아니다 —
  실기기 확인 필요.**
- **다음에 비슷한 "미러 부모 + 다른 회전값" 구조를 다룰 때**: 회전만
  계속 바꿔도 "모양 자체가 이상하다"는 문제가 안 풀리면, 회전이 아니라
  스케일(미러) 쪽을 먼저 의심할 것. Identity 회전에서 양쪽의 실제 본
  좌표를 비교해 "회전 관계(순수 회전, det+1)"인지 "미러 관계(반사 포함,
  det-1)"인지 판별하면 확실하다.
